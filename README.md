...
Must be served over HTTP, not opened as a file. `model-viewer` is an ES module and the
embedded animations are framed documents — the browser blocks both under `file://`.
...
assets/             the 7 design assets (6 animation bundles + the GLB)
vendor/             model-viewer 3.5.0, vendored — no CDN at runtime
...
The hero is a full-bleed 3D canvas with the copy overlaid. Below it, four sections
alternate in a zig-zag:
...
- **Nothing renders off-screen.** The hero's `auto-rotate` is dropped when the hero
  leaves the viewport. The four framed animations drive themselves from
...
Above the fold is roughly 3MB, of which the GLB is 2MB and `model-viewer` 913KB. Each
animation is about 1.1MB and only loads when you scroll near it. Draco or meshopt
compression on the GLB is the lever if the hero needs to be lighter.
